...
function loadUserProfile() {
    try {
        ...
        if (saved) {
            ...
            document.getElementById('privacyMode').checked = userProfile.privacyMode !== false;
        }
    } catch (error) {
...
function updateHistoryDisplay() {
    ...
    for (let i = 0; i < sortedHistory.length; i++) {
        ...
        // Display name based on privacy mode
        let displayName = 'Anonymous User';
        if (result.userName && !result.privacyMode) {
            displayName = result.userName;
        }
        
        let ageDisplay = result.userAge ? ', Age: ' + result.userAge : '';
